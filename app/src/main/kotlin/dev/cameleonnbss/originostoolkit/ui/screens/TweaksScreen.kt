...
fun TweaksScreen(state: ToolkitUiState, viewModel: ToolkitViewModel) {
    ...
    val tweaks = state.visibleTweaks(category).filter {
        query.isBlank() ||
    ...
    Card(
        ...
        Column(Modifier.padding(14.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                if (applied) {
                    Button(onClick = { viewModel.revertTweak(tweak.id) }) { Text("Revert") }
                    OutlinedButton(onClick = { viewModel.applyTweak(tweak, force = true) }) {
                        Text("Re-apply")
                    }
                } else {
                    Button(onClick = { viewModel.applyTweak(tweak) }) { Text("Apply") }
            ...
            if (!state.canWrite) {
                Text(
                    "Shizuku is not ready — applying will fail until it is running.",
                    style = MaterialTheme.typography.labelSmall,
